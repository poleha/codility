...
Gold:
https://app.codility.com/cert/view/certWHFP9D-J7EUYJVMZ363ZKW3/
"""
...
def solution(T):
    ...
    def step(ways, dest, visited=None):
        ...
        for way in ways:
            if way not in visited:
                ...
                continue
            result_path = step(all_ways[way], dest, visited)
            if result_path:
                return [way] + result_path

    maxes = []
    mins = []
    count = 0
    for start in range(B - 1):
        visited = {start}
        dest = start + 1
        current_path = step(all_ways[start], dest, visited)
        maxes.append(max(current_path))
        mins.append(min(current_path))
    ...
    for start in range(B):
        ...
        for end in range(start, B):
            if start == end:
                count += 1
                continue
            current_max = max(prev_max, maxes[end - 1])
            current_min = min(prev_min, mins[end - 1])
            prev_min = current_min
            prev_max = current_max
            if current_min >= start and current_max <= end:
                count += 1
    return count

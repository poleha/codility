...
def solution(H):
    ...
    maxes = OrderedDict(sorted(maxes.items(), key=lambda e: e[0], reverse=True))
    ...
    for i, e in enumerate(H):
        ...
        if maxes[e] == 0:
            del maxes[e]
        right_max = next(iter(maxes.keys()))
        current_s = current_max * (i + 1) + right_max * (L - i - 1)
        min_s = min(min_s, current_s)

    return min_s

example = [3, 1, 4] * 1000000

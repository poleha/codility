...
"""
# Gold
# https://app.codility.com/cert/view/certHQ35HF-H9XXAYZSNX82YWPA/

from collections import deque


def solution(A, B):
    def check_table(dishes):
        for a, b in zip(dishes, B):
            if a == b:
                return False
        return True

    current_dishes = deque(A)
    for i in range(len(A)):
        result = check_table(current_dishes)
        if result:
            return i
        else:
            current_dishes.append(current_dishes.popleft())
    return -1
